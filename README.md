...
修改原因：AI 的原生代码过于“宽容”。增加此校验，能在不抛出阻断异常的前提下，自动过滤脏数据，保障底层统计与关联的准确性。

搜索框回车事件绑定（优化页面交互）：在前端 `app.js` 中补充了对输入框 `keydown` 事件的监听逻辑。
   
修改原因：AI 仅实现了基础的按钮点击搜索，后续通过增加回车触发机制（`e.key === 'Enter'`），提升用户的交互体验。
...
1 对 N： 教师与课程（TEACHERS -> COURSES），一名教师可挂载多门课程。

M 对 N： 学生与课程（STUDENTS <-> COURSES）属于典型的多对多关系。本架构通过建立 ENROLLMENTS（选课记录表）作为中间表，将多对多拆解为两个 1 对 N 关系，逻辑清晰且易于扩展。
...
并发风险分析与解决方案
 ...
 核心风险：并发超卖与数据库行锁争抢
 在抢课高峰期（如 50 人容量的热门课），大量并发请求若先 SELECT 判断容量，再 INSERT 选课记录，极易因为非原子操作导致“超卖”（实际选上的人数超过 50 人）。

简单可行的解决方案：基于数据库行锁的“乐观锁”原子更新
考虑到本项目的规模与轻量化要求，不引入复杂的 Redis 等中间件，而是直接利用 MySQL底层的行排他锁机制，通过一条原子 SQL 完成校验与扣减：
...
联合索引 (student_id, course_id)： 防重兜底，确保在并发重试的情况下，同一个学生不会对同一门课插入两条记录。
